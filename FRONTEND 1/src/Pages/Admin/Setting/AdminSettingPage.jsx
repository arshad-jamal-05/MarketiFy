...
export default function AdminSettingPage() {
  ...
  function postData(e) {
    ...
    formData.append("map1", item.map1);
    formData.append("map2", item.map2);
    ...
    formData.append("twitter", item.twitter);
    formData.append("privacyPolicy", item.privacyPolicy);
    formData.append("termsAndConditions", item.termsAndConditions);
    formData.append("returnPolicy", item.returnPolicy);
    formData.append("cancellationPolicy", item.cancellationPolicy);
    formData.append("refundPolicy", item.refundPolicy);
    dispatch(createSetting(formData));
    toast("Website Settings Has Been Updated Successfully");
    // let formData = new FormData();
    ...
    // formData.append("privacyPolicy", rtePrivacyPolicy.getHTMLCode());
    // formData.append("termsAndConditions", rteTermsAndConditions.getHTMLCode());
    // formData.append("returnPolicy", rteReturnPolicy.getHTMLCode());
    // formData.append("cancellationPolicy", rteCancellationPolicy.getHTMLCode());
    // formData.append("refundPolicy", rteRefundPolicy.getHTMLCode());
    // dispatch(createSetting(formData));
    // toast("Website Settings Has Been Updated Successfully");
  }
  ...
  useEffect(() => {
    ...
    rteTermsAndConditions = new window.RichTextEditor(
      refdivTermsAndConditions.current,
    );
    rteReturnPolicy = new window.RichTextEditor(refdivReturnPolicy.current);
    rteCancellationPolicy = new window.RichTextEditor(
      refdivCancellationPolicy.current,
    );
    ...
    if (SettingStateData.length) {
      ...
      rtePrivacyPolicy.setHTMLCode(
        item.privacyPolicy ? item.privacyPolicy : "",
      );
      ...
      rteCancellationPolicy.setHTMLCode(
        item.cancellationPolicy ? item.cancellationPolicy : "",
      );
